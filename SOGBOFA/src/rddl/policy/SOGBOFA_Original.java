...
public class SOGBOFA_Original extends Policy {
	...
	public SOGBOFA_Original(String instance_name) {
        ...
        // fixed setting 2022:
        // hack to fix graph depth and fix number of gradient updates
//	boolean fixSetting2022 = true;
//	int fixDepthSetting = 9;
//	int fixGradientUpdates = 500;
	...
	TreeExp BuildF(State s) throws Exception {
		...
		for (; h < maxDepth; h++) {	
				...
				if(F.counter != 0) {
					if (!ifFirstStep) {
						...
						if ((numOfNodes + nextStepNodesNum) * updatePerNode * 500 > timeLeft) {
							...
							System.out.println("Time left: " + timeLeft);
							if (fixSetting2022) {
							    System.out.println("fixSetting 2022: avoid h++ and break");
							} else {
							    h++;
							    break;
							}
			...
			if(h!=maxDepth - 1){
				...
				//System.out.println(as);
				if (fixSetting2022) {
				    System.out.println("fixsetting 2022: built to level " + h);
				}
			}
			...
			if ((System.currentTimeMillis() - t0) > _timeAllowed) {
				...
				System.out.println("In Std Mode would be Forced to stop building tree!");
				//break;
				System.out.println("fixSetting 2022: avoid h++ and break");
			    } else {
			        h ++;
	...
	public ArrayList<Double> UpdateAllwtProj(State s, TreeExp F) throws EvalException{
		...
		while(!stopAlgorithm){
			...
			while(!ifConverge && !stopAlgorithm){
				...
				if (fixSetting2022) {
				    // forcing consistent number of updates roughly 500
				    if (updatesIntotal + updatedasCounter > fixGradientUpdates){
					//if(System.currentTimeMillis() - t0 > _timeAllowed * 0.95){
					stopAlgorithm = true;
					break;
				    } else { // 2022 fix bug when no Q imporvement?
					     // seems to be in infinite loop o/w
					if(newQ == initialQ){
...
public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		if (fixSetting2022) {
		    System.out.println("fixSetting 2022: forcing maxDepth to " + fixDepthSetting);
		    maxDepth=fixDepthSetting;
		}
		//counter = 0;
		stopAlgorithm = false;
