...
def collect(domain, alg, simu_type):
    '''
    Collect the results from json file for plotting purpose.
    '''
    inst_lst = []
    mean_lst = []
    std_lst = []
...
if __name__ == "__main__":
    p = configargparse.ArgParser(default_config_files=['../conf/vis_demo.conf'])
    p.add('-d', '--domain', nargs='+', help='do not change it, default to be demo')
    p.add('-i', '--instance', nargs='+', help='demo instance name')
    p.add('-a', '--algorithm', nargs='+', help='demo algorithm name shortcut, including: demo_mfvi_bwd, demo_mfvi_bwd_noS, demo_mdvi_bwd_med, demo_mdvi_bwd_main_all, demo_csvi_bwd, random.')
    ...
    for alg in algs:
        ins_ary, mean_ary, std_ary, rlt_generated = collect(domain, alg, simu_type)
        if (rlt_generated):
            plt.errorbar(ins_ary, mean_ary, std_ary, label=labels[alg], linestyle='dashed', fmt=formats[alg], color=colors[alg], fillstyle = fillstyles[alg], elinewidth=2, linewidth=0.5)
    ...
    plt.savefig(plot_dir + '/{}.pdf'.format(domain), bbox_inches='tight')
    plt.savefig(plot_dir + '/{}.png'.format(domain), bbox_inches='tight')
    plt.close()    