#################################################################################
# Modified based on the reference: https://github.com/krashkov/Belief-Propagation
#   - Changed the message passing order.
#################################################################################
...
class belief_propagation():
    ...
    def belief(self, v_name):
        ...
        print(joint_distribution(incoming_messages))
    ...
    def __init__(self, pgm):
        ...
        for edge in self.__pgm.get_graph().es:
            ...
            if self.__pgm.get_graph().vs[start_index]['is_factor']:
                ...
                # modified
                self.__msg_new[(start_name, end_name)] = factor([end_name],   np.array([1.]*self.__pgm.get_graph().vs[end_index]['rank']))
            else:
                self.__msg[(start_name, end_name)] = factor([start_name], np.array([1.]*self.__pgm.get_graph().vs[start_index]['rank']))
                # modified
                self.__msg_new[(start_name, end_name)] = factor([start_name], np.array([1.]*self.__pgm.get_graph().vs[start_index]['rank']))                
            self.__msg[(end_name, start_name)] = self.__msg[(start_name, end_name)]
            # modified
            self.__msg_new[(end_name, start_name)] = self.__msg[(start_name, end_name)]
            # commented out
